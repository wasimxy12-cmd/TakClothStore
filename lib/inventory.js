const { fetchInventory, saveInventory } = require('./subspace');
...
async function loadInventory() {
    if (process.env.SUBSPACE_API_URL) {
        try {
            ...
            console.error('Unable to read Subspace inventory:', error.message || error);
        }
    }
...
async function persistInventory(inventory) {
    if (!process.env.SUBSPACE_API_URL) {
        throw new Error('No Subspace configuration available for inventory persistence.');
    }
